...
- (void)addItemWithIcon:(NSString *)icon selectedIcon:(NSString *)selected title:(NSString *)title
{
    ...
    for (int i = 0; i<count; i++) {
        ...
        dockItem.tag = i+kDockItemBaseTag; // 绑定标记
        dockItem.frame = CGRectMake(width * i, 0, width, height);
        [dockItem modifyNotifyCounterFrame];
    }
    
    // 默认选中第一个item
    if (count == 1) {
        _selectedItem = (DockItem*)self.subviews[0];
        [self itemClick:item];
    }
}
...
- (void)itemClick:(DockItem *)item
{
    ...
    if ([_delegate respondsToSelector:@selector(dock:itemSelectedFrom:to:)]) {
        [_delegate dock:self itemSelectedFrom:(_selectedItem.tag-kDockItemBaseTag) to:(item.tag-kDockItemBaseTag)];
    }
...
#pragma mark 设置item的消息数
- (void)setItemMessageNumber:(NSString*)str forIndex:(int)index{
    DockItem *item = (DockItem*)[self viewWithTag:index+kDockItemBaseTag];
    [item setMessageNumber:str];
}
@end